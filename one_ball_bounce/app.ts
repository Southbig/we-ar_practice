...
class App {
  ...
  animate = () => {
    ...
    for (let i = 0; i < this.balls.length; i++) {
      const ball_1:Ball = this.balls[i];

      for (let j = 0; j < this.balls.length; j++) {
        const ball_2:Ball = this.balls[j];
        ...
        if (i !== j) {
          ...
          if (d <= ball_1.radius + ball_2.radius) {
            const r:number = Math.atan2(ball_1.y - ball_2.y, ball_1.x - ball_2.x);

            const m1:number = 5;
            const m2:number = 5;

            const v1:number = ball_1.vector();
            const v2:number = ball_2.vector();

            const t1:number = ball_1.theta();
            const t2:number = ball_2.theta();
